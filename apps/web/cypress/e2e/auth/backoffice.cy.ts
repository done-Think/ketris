describe('Backoffice — sign in e sign up de administrador (UI)', () => {
  ...
  it('redireciona para /backoffice/entrar quando um visitante deslogado tenta acessar a área protegida', () => {
    cy.visit('/backoffice/administradores/novo')
    cy.location('pathname').should('eq', '/backoffice/entrar')
  })

  it('bloqueia o acesso de um usuário autenticado que não é ADMIN', () => {
    cy.visit('/backoffice/entrar')
  ...
  it('permite que um ADMIN entre e crie um novo administrador', () => {
    ...
    cy.location('pathname').should('eq', '/backoffice/administradores/novo')

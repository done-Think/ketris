...
function ProposalDetailDialog({
  ...
  const { control, handleSubmit, reset } = useForm<ProposalStatusFormValues>({
    defaultValues: {
      status: proposal?.status ?? 'underReview',
    },
...
export function ProposalsDashboardPage() {
  ...
  const [proposals, setProposals] = useState<DashboardProposal[]>(dashboardProposals)
    ...
    <Box sx={{ width: '100%', px: { xs: 2, md: 3.6 }, py: { xs: 2.4, md: 4.2 } }}>
      <Stack spacing={2.4}>
        ...
        >
          ...
          >
            {['client', 'property', 'value', 'ownerExpectation', 'status'].map((column) => (
              <Typography
                key={column}
                role="columnheader"
                sx={{
